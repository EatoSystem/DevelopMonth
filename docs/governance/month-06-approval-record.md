# Month 6 content approval record

## Current status

**Draft—do not represent as professionally approved.**

The Month 6 support pack is suitable for product and usability testing with
fictional data. It is not approved for reliance as clinical, feeding,
safer-sleep, urgent-care, or developmental guidance.

## Version record

| Field | Value |
| --- | --- |
| Content | Month 6: Starting solids and movement |
| Current version | Draft 0.1 |
| Product owner | Unassigned |
| Editorial owner | Unassigned |
| Professional review coordinator | Unassigned |
| Jurisdiction | Ireland proposed; not yet confirmed |
| Last edited | 5 July 2026 |
| Approved for product testing | Yes, with fictional profile and static disclaimer |
| Approved for parent reliance | No |
| Next review date | Before any public-content release |

## Required scoped reviews

| Area | Reviewer qualification/scope | Reviewer | Status | Required evidence/output |
| --- | --- | --- | --- | --- |
| General child health | GP, paediatrician, or appropriate public-health professional | Unassigned | Pending | Claims, boundaries, signposting |
| Complementary feeding | Paediatric dietitian, feeding specialist, or relevant qualified professional | Unassigned | Pending | Readiness, allergens, choking prevention, nutrition |
| Safer sleep | Qualified reviewer working to current Irish guidance | Unassigned | Pending | Current safer-sleep wording and links |
| Movement | Paediatric physiotherapist/occupational therapist or relevant specialist | Unassigned | Pending | Positioning, movement ideas, adaptations |
| Communication/play | Speech and language therapist, developmental specialist, or relevant researcher | Unassigned | Pending | Communication and shared-attention framing |
| Parent-facing editorial | Plain-language and accessibility reviewer | Unassigned | Pending | Clarity, inclusion, reading level |
| Privacy/personalisation | Data-protection and child-privacy review | Unassigned | Pending | Companion and summary boundaries |

## Approval checklist

- [ ] Sources recorded with access dates
- [ ] Irish jurisdiction confirmed
- [ ] Feeding language professionally reviewed
- [ ] Allergen and choking-prevention links approved
- [ ] Safer-sleep language reviewed
- [ ] Movement activities reviewed
- [ ] Communication/play framing reviewed
- [ ] Signposting model reviewed
- [ ] Urgent-care boundary reviewed
- [ ] Accessibility and inclusive-language review completed
- [ ] Parent testing completed
- [ ] Professional testing completed
- [ ] Critical/high findings resolved
- [ ] Content version updated
- [ ] Named approvers sign within scope
- [ ] Next review date assigned

## Reviewer sign-off

Each reviewer signs only for the stated scope.

| Reviewer | Qualification/organisation | Scope | Decision | Required changes | Date | Conflict declared |
| --- | --- | --- | --- | --- | --- | --- |
| — | — | — | Pending | — | — | — |

Decision values:

- Approved within scope
- Approved within scope after listed changes
- Changes required
- Outside reviewer scope

## Release decision

No release decision has been made. A qualified professional review cannot be
substituted by an internal product review or automated system.

